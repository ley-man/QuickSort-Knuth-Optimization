...
class QuickSort {
 ...
 public:
  ...
  void knuthQuickSort(int *array, int n, int k) {
    ...
    cout<<"Runtime in microseconds: "<<del_t<<" for k = "<<k<<'\n';
  }

  float knuthQuickSortTimed(int *array, int n, int k) { //returns time for Golden Section Search
    // Creates a copy of the orginal array so that f(c) and f(d) can be evaluated  
    // iteratively for Golden Section Search
...
int main() {
  ...
  int k_opt = optimizeK(n); //Extra credits
